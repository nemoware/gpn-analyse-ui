...
async function generateConclusion(audit) {
  ...
  const conclusion = {};
  conclusion.intro = `В целях исполнения решения Совета директоров ОАО «Газпром» от 05.09.2013 г. № 2243, поручения начальника Департамента по управлению имуществом и корпоративными отношениями ОАО «Газпром» Михайловой Е.В. от 25.11.2013 г. № 01/05-9226, руководством ПАО «Газпром нефть» (далее – «Компания», «ГПН») было принято решение о проведении Департаментом корпоративного и проектного сопровождения ГПН (далее – «ДКиСП») с ${moment(
    audit.auditStart
  ).format('MMMM YYYY')} г. по ${moment(audit.auditEnd).format(
    'MMMM YYYY'
  )} аудита практики корпоративного управления (далее – «Корпоративный аудит») в дочерних обществах (далее – «ДО») Компании, в частности ООО «Арктика Медиа» (далее – «Общество», «краткое наименование (далее –${
    audit.subsidiaryName
  })»).
Цель Корпоративного аудита – выявление сильных и слабых сторон существующей в  ${
    audit.subsidiaryName
  } практики корпоративного управления в сравнении со стратегическими целями Компании; подтверждение соблюдения ${
    audit.subsidiaryName
  } требований системы корпоративного управления; выявление задач, которые необходимо решить в области корпоративного управления; подготовка конкретных рекомендаций комплексного плана по совершенствованию системы корпоративного управления ${
    audit.subsidiaryName
...
В период с ${moment(audit.createDate).format('DD.MM.YYYY')} г. по ${
  ...
  } г. на основании обращения Заместителя генерального директора по правовым и корпоративным вопросам ПАО «Газпром нефть» Илюхиной Е.А. № НК-ХХ от ХХ.ХХ.ХХХХ г. (Приложение № 1) ДКИСП был проведен Корпоративный аудит ${
    audit.subsidiaryName
  }.
Процедура Корпоративного аудита включала в себя:
1. Получение информации об исходном состоянии корпоративного управления.
2. Документальную проверку ${audit.subsidiaryName}, в том числе:
2.1. экспертизу процедуры хранения оригиналов/копий учредительных документов ${
    audit.subsidiaryName
  };
...
3. Полную проверку, представленных документов ${
    audit.subsidiaryName
...
По результатам проведенного Корпоративного аудита подготовлен настоящий отчет, содержащий основные результаты Корпоративного аудита, а также рекомендации ДКИСП относительно усовершенствования практики корпоративного управления ${
    audit.subsidiaryName
  }.\n`;
  conclusion.shortSummary = `Ниже перечислены наиболее существенные сильные стороны и недостатки системы корпоративного управления ${audit.subsidiaryName} согласно результатам Корпоративного аудита, проведенного ДКиСП.\n`;
  conclusion.strengths = `Сформирован состав Совета директоров ${audit.subsidiaryName} из членов, обладающих соответствующими знаниями, навыками, профессиональным и практическим опытом, необходимыми для успешного выполнения функций Совета директоров по управлению Обществом и принятия решений по вопросам, отнесенным к компетенции Совета директоров.\n`;
